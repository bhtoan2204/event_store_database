...
func GenerateInitRoutes(protos []*model.Proto) error {
	...
	template, err := utils.ReadFileAsString("scaffolds/template/routing_init.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	paramsRoutingTemplate, err := utils.ReadFileAsString("scaffolds/template/routing_init_params.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	funcsRoutingTemplate, err := utils.ReadFileAsString("scaffolds/template/routing_init_funcs.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	for _, proto := range protos {
		...
		for _, service := range proto.Services {
			serviceName := strings.ReplaceAll(utils.ToPascalCase(proto.Module)+utils.ToPascalCase(service.Name), "Service", "")
			paths := strings.Split(service.Path, ",")
			// name := utils.ToPascalCase(proto.Module + model.ServicePath[service.Path])
			if len(paths) > 1 {
		...
		if _, found := funcs[proto.Module]; !found {
			if module := utils.Find(model.IGNORE_PROTO_MODULES, func(m string) bool {
				return m == proto.Module
			}); module == "" {
				frouting := funcsRoutingTemplate
				frouting = strings.ReplaceAll(frouting, "<module_name>", utils.ToPascalCase(proto.Module))
	...
	err = utils.WriteFileWithFormat([]byte(template), INIT_SERVER_ROUTES_PATH)
	if err != nil {
