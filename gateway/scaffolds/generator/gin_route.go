...
func GenerateGinRoutes(protos []*model.Proto) error {
	...
	template, err := utils.ReadFileAsString("scaffolds/template/gin_routes.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	routingTemplate, err := utils.ReadFileAsString("scaffolds/template/gin_route_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	pubRoutingTemplate, err := utils.ReadFileAsString("scaffolds/template/gin_route_entry_public.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	entryTemplate, err := utils.ReadFileAsString("scaffolds/template/route_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	err = utils.WriteFileWithFormat([]byte(template), GIN_ROUTES_PATH)
	if err != nil {
