...
func generateHandlers(proto *model.Proto, messages map[string]map[string]*model.Message) ([]*model.Handler, error) {
	...
	for i := 1; i < len(segments)-1; i++ {
		...
		err := utils.CreateFolderIfNotExist(folderPath)
		if err != nil {
	...
	if segments[len(segments)-2] != proto.Package {
		...
		err := utils.CreateFolderIfNotExist(parentFolderPath)
		if err != nil {
...
func generatePostHandler(handler *model.Handler, requestType *model.Message) error {
	...
	if handler.Accept == "multipart/form-data" {
		templatePath = "scaffolds/template/handler_post_file.tmpl"
	} else if handler.Type == "webhook" {
		templatePath = "scaffolds/template/handler_post_webhook.tmpl"
	} else {
		templatePath = "scaffolds/template/handler_post.tmpl"
	}

	template, err := utils.ReadFileAsString(templatePath)
	if err != nil {
	...
	handlerNameLower1st := utils.LowerFirstChar(handler.Name)
	...
	template = strings.ReplaceAll(template, "<request_type_lower_1st>", utils.LowerFirstChar(handler.RequestType))
	...
	// handle params
	paramDecTemplate, err := utils.ReadFileAsString("scaffolds/template/param_declaration.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	paramParsingIdTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_id.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingFileTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_file.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingEnumTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_enum.tmpl")
	if err != nil {
	...
	for _, field := range requestType.Fields {
		...
		if field.Location == "path" {
			...
			parsing = strings.ReplaceAll(parsing, "<param_name_cammel_case>", utils.SnakeToCamel(field.Name))
			paramContext := "Query"
			if field.Location == "path" {
				paramContext = "Param"
			}
			...
			parsing = strings.ReplaceAll(parsing, "<param_name_upper_case_word>", utils.ConvertSliceStrToUCWord(strings.Split(field.Name, "_")))
	...
	fpath := handler.FolderPath + "/" + utils.ToSnakeCase(handlerNameLower1st) + ".go"
...
func generatePutHandler(handler *model.Handler, requestType *model.Message) error {
	template, err := utils.ReadFileAsString("scaffolds/template/handler_put.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	err = os.WriteFile(handler.FolderPath+"/"+utils.ToSnakeCase(handlerNameLower1st)+".go", []byte(template), 0644)
	if err != nil {
		fmt.Println("failed to write handler file:", err)
		return err
	}
...
func generateGetHandler(handler *model.Handler, requestType *model.Message) error {
	template, err := utils.ReadFileAsString("scaffolds/template/handler_get.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	paramParsingIntTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_int.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingArrIntTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_arr_int.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingStringTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_string.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	paramParsingArrStringTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_arr_string.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingBoolTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_bool.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	paramParsingGGTimestampTemplate, err := utils.ReadFileAsString("scaffolds/template/param_parsing_gg_timestamp.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
...
func GenerateRoutingRegistry(handlers []*model.Handler, handlerFolderPaths []string) error {
	...
	template, err := utils.ReadFileAsString("scaffolds/template/routing_registry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	handlerImportTemplate, err := utils.ReadFileAsString("scaffolds/template/handler_import.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	routingConfigTemplate, err := utils.ReadFileAsString("scaffolds/template/routing_config.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	err = utils.WriteFileWithFormat([]byte(template), ROUTING_REGISTRY_PATH)
	if err != nil {
