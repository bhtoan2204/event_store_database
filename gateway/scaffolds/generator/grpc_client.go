...
const SERVICE_CLIENT_REGISTRY_PATH = "application/routing/delivery/service/remote_service_client_registry.go"

func GenerateGrpcClients(protos []*model.Proto) error {
	for _, proto := range protos {
		generateGrpcClients(proto) //nolint
	}
...
func generateGrpcClients(proto *model.Proto) error {
	for _, service := range proto.Services {
		generateGrpcClient(proto, service) //nolint
	}
...
func generateGrpcClient(proto *model.Proto, service *model.Service) error {
	...
	for i, s := range segments {
		segments[i] = utils.ToSnakeCase(s)
	}

	template, err := utils.ReadFileAsString("scaffolds/template/grpc_client.tmpl")
	if err != nil {
	...
	template = strings.ReplaceAll(template, "<config_name>", configName)
	template = strings.ReplaceAll(template, "<service_name>", service.Name)
	template = strings.ReplaceAll(template, "<service_name_lower_1st>", utils.LowerFirstChar(service.Name))

	methodConfigTemplate, err := utils.ReadFileAsString("scaffolds/template/grpc_method_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	for _, api := range service.Apis {
		...
		entry = strings.ReplaceAll(entry, "<api_name_lower_1st>", utils.LowerFirstChar(api.Name))
		methodEntries = append(methodEntries, entry)
	}
	template = strings.ReplaceAll(template, "<method_entries>", strings.Join(methodEntries, "\n"))

	apiInvocationTemplate, err := utils.ReadFileAsString("scaffolds/template/grpc_api_invocation.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	for _, api := range service.Apis {
		...
		apiInvocation = strings.ReplaceAll(apiInvocation, "<service_name_lower_1st>", utils.LowerFirstChar(service.Name))
		apiInvocation = strings.ReplaceAll(apiInvocation, "<api_name_lower_1st>", utils.LowerFirstChar(api.Name))
	...
	name := utils.ToSnakeCase(utils.LowerFirstChar(service.Name) + "Client.go")
	err = utils.WriteFileWithFormat([]byte(template), GRPC_CLIENT_PARENT_PATH+"/"+name)
	if err != nil {
...
func GenerateServiceClientRegistry(protos []*model.Proto) error {
	...
	template, err := utils.ReadFileAsString("scaffolds/template/service_client_registry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	clientEntryTemplate, err := utils.ReadFileAsString("scaffolds/template/grpc_service_client_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
